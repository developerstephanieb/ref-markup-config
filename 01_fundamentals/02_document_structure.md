...
A LaTeX document is divided into two distinct sections: the preamble for global configuration, and the body for content organization.

---

[The Preamble](#the-preamble)   
[Customizing the Title (the titling package)](#customizing-the-title-the-titling-package)     
...
- `\usepackage{titling}`: Offers detailed control over the layout of the title block generated by `\maketitle`.
...
- `\begin{document}`: Encloses all visible content.
...
- `\input{<filename>}`: Inserts the content of another file (`filename.tex`) at the specified location. The external file should not contain its own preamble or document environment.
